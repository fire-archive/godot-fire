...
# Detect modules.
opts.Add("custom_modules", "Directory path containing custom modules to build along with built-in modules.", "")

modules_detected = {}
module_search_paths = ["modules"]  # Built-in path.

if ARGUMENTS.get("custom_modules"):
    paths = ARGUMENTS.get("custom_modules").split(os.pathsep)
    for p in paths:
        try:
